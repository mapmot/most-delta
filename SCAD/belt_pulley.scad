...
T10_belt_profile		=	6;
AT5_belt_profile		=	7;
HTD_3mm_belt_profile	=	8;
HTD_5mm_belt_profile	=	9;
HTD_8mm_belt_profile	=	10;
GT2_2mm_belt_profile	=	11;
GT2_3mm_belt_profile	=	12;
GT2_5mm_belt_profile	=	13;

belt_data=[
//		name,		pitch,	depth,	width,	backing
//		==========	======	======	======	=======
	[	"MXL",		2.032,	0.508,	1.321,	0.64	],
	[	"40 D.P.",	0,		0.457,	1.226,	0		],
	[	"XL",		0,		1.27,	3.051,	0		],
	[	"H",		0,		1.905,	5.359,	0		],
	[	"T2.5",		2.5,	0.7,	1.678,	0.6		],
	[	"T5",		5,		1.19,	3.264,	1		],
	[	"T10",		10,		2.5,	6.13,	2		],
	[	"AT5",		0,		1.19,	4.268,	0		],
	[	"HTD 3mm",	0,		1.289,	2.27,	0		],
	[	"HTD 5mm",	0,		2.199,	3.781,	0		],
	[	"HTD 8mm",	0,		3.607,	6.603,	0		],
	[	"GT2 2mm",	2,		0.764,	1.494,	0.6		],
	[	"GT2 3mm",	0,		1.169,	2.31,	0		],
	[	"GT2 5mm",	0,		1.969,	3.952,	0		]
];

function belt_profile_name(belt_profile) = belt_data[belt_profile][0];
function belt_profile_pitch(belt_profile) = belt_data[belt_profile][1];
function belt_profile_depth(belt_profile) = belt_data[belt_profile][2];
function belt_profile_width(belt_profile) = belt_data[belt_profile][3];
function belt_profile_backing(belt_profile) = belt_data[belt_profile][4];

function tooth_spacing_curvefit (teeth,b,c,d)
	= ((c * pow(teeth,d)) / (b + pow(teeth,d))) * teeth ;

function tooth_spacing(teeth,tooth_pitch,pitch_line_offset)
	= (2*((teeth*tooth_pitch)/(3.14159265*2)-pitch_line_offset)) ;

function pulley_diameter(belt_profile = 5, teeth = 8) = 
	belt_profile==0	? tooth_spacing (teeth,2.032,0.254) :
	belt_profile==1	? tooth_spacing (teeth,2.07264,0.1778) :
	belt_profile==2	? tooth_spacing (teeth,5.08,0.254) :
	belt_profile==3	? tooth_spacing (teeth,9.525,0.381) :
	belt_profile==4	? tooth_spacing_curvefit (teeth,0.7467,0.796,1.026) :
	belt_profile==5	? tooth_spacing_curvefit (teeth,0.6523,1.591,1.064) :
	belt_profile==6	? tooth_spacing (teeth,10,0.93) :
	belt_profile==7	? tooth_spacing_curvefit (teeth,0.6523,1.591,1.064) :
	belt_profile==8	? tooth_spacing (teeth,3,0.381) :
	belt_profile==9	? tooth_spacing (teeth,5,0.5715) :
	belt_profile==10	? tooth_spacing (teeth,8,0.6858) :
	belt_profile==11	? tooth_spacing (teeth,2,0.254) :
	belt_profile==12	? tooth_spacing (teeth,3,0.381) :
	belt_profile==13	? tooth_spacing (teeth,5,0.5715) :
	0;

//pulley(belt_profile=11, teeth=40, retainer=1, idler=1, pulley_t_ht=6, pulley_b_ht=6, pulley_b_dia=20);

//translate([0,-12.5,0]) color("red") belt(belt_profile = GT2_2mm_belt_profile, belt_length = 30, belt_width = 6);
...
//belt_system(GT2_2mm_belt_profile, 40, 36, 9, 305, 6);

module belt_arc(belt_profile = 5, teeth = 8, belt_width = 6) {
	d_p = pulley_diameter(belt_profile = belt_profile, teeth = teeth);
	b_b_t = belt_profile_backing(belt_profile) * 2;
	half_cube = d_p + 2 * b_b_t;
	difference() {
		difference() {
			cylinder(d = b_b_t + d_p, teeth = teeth, h = belt_width);
			pulley(belt_profile=belt_profile, teeth=teeth, motor_shaft = 0, retainer=0, idler=0, pulley_t_ht=belt_width+2, pulley_b_ht=-1, pulley_b_dia=0, no_of_nuts=0);
		}
		translate([-half_cube/2, 0, -half_cube/2]) cube(half_cube);
	}
}

module belt_system(b_profile, p1_teeth, p2_teeth, c_c_offset, b_length, b_width) {
	
	d1 = pulley_diameter(belt_profile = b_profile, teeth = p1_teeth);
	d2 = pulley_diameter(belt_profile = b_profile, teeth = p2_teeth);

	b_pitch = belt_profile_pitch(b_profile);
	...
	belt_side = floor(((b_length - c1 / 2 - c2 / 2) / 2) / b_pitch) * b_pitch;
	
	c_c_distance = sqrt(pow(belt_side, 2) - pow(d2-d1, 2));


	echo (str("Pulley c-c distance = "), c_c_distance);

		belt_arc(belt_profile = b_profile, teeth = p1_teeth, belt_width = b_width);
	translate([c_c_offset, c_c_distance, 0])
	rotate(180, 0, 0)
		belt_arc(belt_profile = b_profile, teeth = p2_teeth, belt_width = b_width);
	
	//straight part
	translate([d1 / 2, 0, 0])
	rotate([0,0,90-atan((d2-d1+c_c_offset*2)/2/c_c_distance)])
		belt_line(belt_profile = b_profile, belt_width = b_width, belt_length = belt_side);
	//sloped part
	translate([-d1 / 2, 0, b_width])
	rotate([180,0,90+atan((d2-d1-c_c_offset*2)/2/c_c_distance)])
		belt_line(belt_profile = b_profile, belt_width = b_width, belt_length = belt_side);

}

module pulley(
	belt_profile = 5,		// 0=MXL 1=40DP 2=XL 3=H 4=T2.5 5=T5 6=T10 7=AT5 8=HTD_3mm 9=HTD_5mm 10=HTD_8mm 11=GT2_2mm 12=GT2_3mm 13=GT2_5mm
	...
	pulley_OD = pulley_diameter(belt_profile = belt_profile, teeth = teeth);
	tooth_depth = belt_profile_depth(belt_profile);
	tooth_width = belt_profile_width(belt_profile);
	echo (str("Belt type = ",belt_profile_name(belt_profile),"; Number of teeth = ",teeth,"; Pulley Outside Diameter = ",pulley_OD,"mm "));
	...
	difference()
	 {	 
		...
		difference()
			...
			single_tooth(tooth_profile = belt_profile, width = pulley_t_ht+2);
...
module belt_line(belt_profile = 5, belt_length = 10, belt_width = 6)
{

tooth_pitch = belt_profile_pitch(belt_profile);
backing_thickness = belt_profile_backing(belt_profile);

n = belt_length / tooth_pitch;

for( i = [0:n])
	{
		union(){
			translate([tooth_pitch*i,0,0])single_tooth(tooth_profile = belt_profile, width = belt_width);
...
module single_tooth(tooth_profile = 5, width = 2) {
//MXL
	if ( tooth_profile == 0 ) {
